                    ...
                    <tbody>
                        @foreach ($trains as $item)
                                <tr class="@if($item->cancellato) table-danger @elseif(!$item->cancellato && !$item->in_orario) table-warning  @endif ">
                                  <th scope="row">{{$item->azienda}} @if($item->cancellato)<span class="text-danger">  CANCELLATO </span> @elseif(!$item->cancellato && !$item->in_orario) <span class="text-warning">  IN RITARDO </span> @endif</th>
                                  <td>{{$item->stazione_partenza}}-{{$item->stazione_arrivo}}</td>
